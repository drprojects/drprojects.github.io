...
def twemoji_code(emoji: str) -> str:
    """Base codepoint of an emoji, as twemojis names them.

    Variation selectors and zero-width joiners are dropped: "\u2601\ufe0f"
    is the glyph twemojis calls 2601.
    """
    for ch in str(emoji):
        cp = ord(ch)
...
def validate_empty_entries(data):
    """Catch a stray "-" in a YAML list, which parses to a null entry.

    Without this the failure surfaces much later as an opaque Jinja error such
    as "'None' has no attribute 'year'".
    """
    bad = [
        f"{name}.yml: entry {i + 1} is empty (a stray '-'?)"
        for name in DATASETS
        for i, item in enumerate(data[name])
        if isinstance(data[name], list) and item is None
    ]
    if bad:
        for line in bad:
            print(f"    {line}", file=sys.stderr)
        sys.exit(f"\n{len(bad)} empty entr(y/ies); nothing was written.")
...
def build(offline: bool, keep_tex: bool) -> int:
    ...
    env.filters.update(tex=tex, md2tex=md2tex, strip_emoji=strip_emoji,
                       twemoji_code=twemoji_code)

    validate_empty_entries(data)
